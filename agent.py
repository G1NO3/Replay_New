...
class Policy(nn.Module):
    ...
    @nn.compact
    def __call__(self, hippo_hidden, theta, s_a_r):
        hippo_info = nn.tanh(nn.Dense(self.bottleneck_size)(hippo_hidden)) 
        input = jnp.concatenate((s_a_r, theta, hippo_info), axis=-1)
        new_theta = nn.tanh(nn.Dense(self.theta_hidden_size)(input))#[104->32]
        policy = nn.Dense(self.output_size)(nn.tanh(nn.Dense(self.output_size)(new_theta)))# [4->4]
        value = nn.Dense(1)(nn.tanh(nn.Dense(64)(new_theta)))# [32->64->1]
        # goal_location = nn.Dense(2)(nn.tanh(nn.Dense(64)(new_theta)))# [32->64->2]
        return new_theta, (policy, value, hippo_info)
...
class Hippo(nn.Module):
    ...
    @nn.compact
    def __call__(self, hipp_hidden, theta, s_a_r):
        # pfc_input [n, d](-1~1), encoder_input(obs_embed[n, d], action_embed[n, d])(-1~1)), hipp_hidden[n, d](-1~1)
        # rewards[n, 1]
        theta_info = nn.tanh(nn.Dense(self.bottleneck_size)(theta))  # [32->4->8]
